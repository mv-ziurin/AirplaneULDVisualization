...
export class Camera {
  ...
  setPerspective(verticalFov, aspectRatio, near, far) {
    ...
    this.projection = new Transformation();
    this.projection.fields[0] = 1 / width_div_2n;
    this.projection.fields[5] = 1 / height_div_2n;
  ...
  use(shaderProgram) {
    this.projection.sendToGpu(shaderProgram.gl, shaderProgram.projection, false);
    this.getInversePosition().sendToGpu(shaderProgram.gl, shaderProgram.view, false);
  }
}
